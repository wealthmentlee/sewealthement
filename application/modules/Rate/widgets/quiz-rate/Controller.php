...
class Rate_Widget_QuizRateController extends Engine_Content_Widget_Abstract
{
  private $item_ids = array();

  public function indexAction()
  {
    $this->view->item_type = $item_type = 'quiz';
    ...
    $select = $quizzesTbl->select()->where('quiz_id IN (?)', $this->item_ids);

    $items = $quizzesTbl->fetchAll($select);

 ...
 * @version    $Id: AuthController.php 2011-02-14 06:58:57 mirlan $
 * @author     Mirlan
 */
...
class User_AuthController extends Core_Controller_Action_Standard
{
  ...
  public function loginAction()
  {
    ...
    // Make form
    $this->view->form = $form = new Mobile_Form_Login();
		$form->removeElement('remember');
		$forgot = $form->getElement('forgot')->setDecorators(array('ViewHelper'));
		$form->removeElement('forgot');
		$form->getDisplayGroup('buttons')->addElement($forgot);
    ...
    if( !$user->verified || !$user->enabled ) {
      ...
      if( !empty($user) && !$user->verified ) {
        $resend_url = $this->_helper->url->url(array('action'=>'home'), 'user_extended', true);
        $error .= ' ';
    ...
    if( $viewer->getIdentity() )
    {
      ...
      $viewer->save();
      Engine_Api::_()->getDbtable('actions', 'activity')->addActivity($viewer, $viewer, 'login', null, array('is_mobile' => true));
    }
  ...
  public function logoutAction()
  {
    ...
    // Test activity @todo remove
    Engine_Api::_()->getDbtable('actions', 'activity')->addActivity($viewer, $viewer, 'logout', null, array('is_mobile' => true));

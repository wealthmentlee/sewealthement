<?php
class Apptouch_Plugin_Signup_Photo extends Core_Plugin_FormSequence_Abstract
{
  protected $_name = 'account';

  protected $_formClass = 'Apptouch_Form_Signup_Photo';
      ...
      try {
        ...
        if( $facebook && $settings->core_facebook_enable ) {
          ...
          if( Engine_Api::_()->apptouch()->isApp() ) {
            $apiInfo = Engine_Api::_()->apptouch()->getFBApiInfo();
          } else {
    ...
    if( !empty($_SESSION['twitter_signup']) ) {
      try {
        ...
        if( $twitter && $settings->core_twitter_enable ) {
          if( Engine_Api::_()->apptouch()->isApp() ) {
            $accountInfo = Engine_Api::_()->apptouch()->getTWApiInfo();
...
//    }
  ...
  public function onSubmit(Zend_Controller_Request_Abstract $request)
  {
    ...
    if( $this->getForm()->isValid($request->getPost()) &&
      $skip != "skipForm" &&
      ($uploadPhoto == true || $photo = $this->getPicupFiles($request, 'Filedata')) &&
      $finishForm != "finish" ) {
      $this->getSession()->data = $this->getForm()->getValues();
      $this->getSession()->Filedata = $this->getForm()->Filedata->getFileInfo();
      if($photo = $this->getPicupFiles($request, 'Filedata')){
        $this->_resizeImages($photo[0]);
      } else{
      ...
      return true;
    } else if(!Engine_Api::_()->getApi('settings',  'core')->getSetting('user.signup.photo', 0) && $photoIsRequired && ($skip == "skipForm" ||
  ...
  public function getPicupFiles(Zend_Controller_Request_Abstract $request, $name = null)
  {

    $files = Zend_Json::decode($request->getParam('picup_files'), '{}');
  ...
  protected function _fetchImage($photo_url)
  {
    ...
    $logfile = APPLICATION_PATH_TMP . DS . 'curl_log.txt';
  ...
  {
    $step_table = Engine_Api::_()->getDbtable('signup', 'apptouch');
    $step_row = $step_table->fetchRow($step_table->select()->where('class = ?', 'Apptouch_Plugin_Signup_Photo'));

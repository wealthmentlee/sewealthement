 ...
 * @license    http://www.socialengine.com/license/
 */
class Apptouch_Plugin_Signup_Account extends Core_Plugin_FormSequence_Abstract
{
  protected $_name = 'account';

  protected $_formClass = 'Apptouch_Form_Signup_Account';
...
//        || !empty($_SESSION['janrain_signup'])
      ...
      if(!empty($_SESSION['facebook_signup']) ) {
        try {
          ...
          Engine_Api::_()->getDbTable('settings', 'core')->setSetting('test.test', $facebook && $settings->core_facebook_enable);
...
//              $facebook->api('/me'); //
            ...
            if( ($usernameEl = $form->getElement('username')) && !$usernameEl->getValue() ) {
              $usernameEl->setValue(preg_replace('/[^A-Za-z]/', '', $apiInfo['username']));
            }
          ...
          if( $twitter && $settings->core_twitter_enable ) {
            if(Engine_Api::_()->apptouch()->isApp()){
              ...
              $this->getForm()->populate(array(
                //'email' => $apiInfo['email'],
                'username' => preg_replace('/[^A-Za-z]/', '', $accountInfo['screen_name']), // $accountInfo->screen_name
                // 'timezone' => $accountInfo->utc_offset, (doesn't work)
                //'language' => $accountInfo->lang, //todo why comented?
              ));
            }
            else {
    ...
    if(!empty($_SESSION['facebook_signup']) ) {
      try {
        ...
        if( $facebook && $settings->core_facebook_enable ) {
          ...
          if( Engine_Api::_()->apptouch()->isApp() ) {
            $info = Engine_Api::_()->apptouch()->getFBApiInfo();
        ...
        if( $twitter && $twitterOauth && $settings->core_twitter_enable ) {
          if( Engine_Api::_()->apptouch()->isApp() ) {
            ...
            $twitterTable->insert(array(
              'user_id' => $user->getIdentity(),
              'twitter_uid' => $accountInfo['user_id'],
              'twitter_token' => $accountInfo['oauth_token'],
              'twitter_secret' => $accountInfo['oauth_token_secret'],
            ));
          } else {
            $accountInfo = $twitter->account->verify_credentials();
            $twitterTable->insert(array(

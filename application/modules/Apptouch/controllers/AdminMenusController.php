 ...
 * @license    http://www.socialengine.net/license/
 */
class Apptouch_AdminMenusController extends Core_Controller_Action_Admin
{
  protected $_menus;
  ...
  {
    // Get list of menus
    $menusTable = Engine_Api::_()->getDbtable('menus', 'apptouch');

    $this->view->menus = $this->_menus = $menusTable->getEnabledMenus();

    $this->_enabledModuleNames = Engine_Api::_()->apptouch()->getEnabledModuleNames();
  }

  public function indexAction()
  {
    $this->view->name = $name = $this->_getParam('name', 'core_dashboard');
    ...
    // Get menu items
    $menuItemsTable = Engine_Api::_()->getDbtable('menuItems', 'apptouch');
  ...
  public function createAction()
  {
    ...
    // Get form
    $this->view->form = $form = new Apptouch_Form_Admin_MenuItemCreate();
    ...
    if( in_array($name, $tabMenus)) {
      $form->removeElement('icon');
    }

    if( $name == 'core_dashboard' ) {
      $form->getElement('icon')->setDescription("This Icon Will be Shown in Applicatoin Dashboard.");
    }
    ...
    $menuItem->data_attrs = Zend_Json_Encoder::encode(array('data-icon' => $icon, 'data-ajax' => $ajax));
    $menuItem->save();
  ...
  public function onOffAction()
  {
    ...
    $this->view->enabled = $menuItem->enabled = !$menuItem->enabled;
    $menuItem->save();
  }
  public function editAction()
  {
    ...
    // Get form
    $this->view->form = $form = new Apptouch_Form_Admin_MenuItemEdit();
    ...
    $data = array();
    if($menuItem->data_attrs)
      $data = Zend_Json_Decoder::decode($menuItem->data_attrs);

    if( !empty($data['data-rel']) ) {
      $form->target->setValue($data['data-rel']);
    }
    if( !empty($data['data-icon']) ) {
      $form->icon->setValue($data['data-icon']);
    }
    if( !empty($data['data-ajax']) ) {
      $form->ajax->setValue($data['data-ajax']);
    }

    $isDivider = false;
    if( !empty($data['role']) && $data['role'] == 'list-divider' ) {
      $form->removeElement('target');
      $form->removeElement('ajax');
    ...
    if( in_array($menuItem->menu, $tabMenus) || $isDivider) {
      $form->removeElement('icon');
    }

    if( $menuItem->menu == 'core_dashboard' && !$isDivider) {
      $form->getElement('icon')->setDescription("This Icon Will be Shown in Applicatoin Dashboard.");
    }
    ...
    if(Engine_Api::_()->hasModuleBootstrap('appmanager')){
      $menuItem->appenabled = !empty($values['appenabled']);
      unset($values['appenabled']);
    }
    unset($values['label']);
    unset($values['enabled']);
    ...
    if (!empty($values['target'])) {
      $data['data-rel'] = 'dialog';
      unset($values['target']);
    }
    if($values['icon']) {
      $data['data-icon'] = $values['icon'];
      unset($values['icon']);
    }
    if($values['ajax']) {
      $data['data-ajax'] = $values['ajax'];
      unset($values['ajax']);
    }
    ...
    $menuItem->data_attrs = Zend_Json_Encoder::encode($data);
  ...
  public function orderAction()
  {
    ...
    $table = Engine_Api::_()->getDbtable('menuItems', 'apptouch');

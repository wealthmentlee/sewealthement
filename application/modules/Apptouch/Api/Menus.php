...
class Apptouch_Api_Menus extends Core_Api_Abstract
{
  ...
  public function getMenuParams($name, array $options = array(), $activeItem = null)
  {
    ...
    foreach ($menu as $row)
    {
      ...
      if(Engine_Api::_()->apptouch()->isApp())
        if (isset($row->appenabled) && !$row->appenabled) {
          continue;
        }
      else
        if ((isset($row->enabled) && !$row->enabled) || (isset($row->appenabled) && !$row->appenabled)) {
          continue;
        }
      ...
      if (!empty($row->plugin)) {
        ...
        if ($result === true) {
          ...
          if (array_values($result) !== $result) {
            $page = $result;
            $page['data_attrs'] = Zend_Json::decode($row->data_attrs);
          }
          // Multi
          else
          {
            ...
            foreach ($result as $key => $value)
            {
              if (is_numeric($key)) {
                ...
                $value['data_attrs'] = Zend_Json::decode($row->data_attrs);
        ...
        } else if ($result instanceof Zend_Db_Table_Row_Abstract && $result->getTable() instanceof Apptouch_Model_DbTable_MenuItems) {
      ...
      array_splice($pages , $page['order'], 0, array($page));
//      $pages[] = $page;
    }
    return $pages;
  }
  ...
  public function getMenu($name)
  {
    ...
    $enabledModuleNames = Engine_Api::_()->apptouch()->getEnabledModuleNames();

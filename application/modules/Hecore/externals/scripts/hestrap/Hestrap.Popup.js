...
description: A simple Popup class for the Twitter Hestrap CSS framework.
...
provides: [Hestrap.Popup]

...
*/

Hestrap.Popup = new Class({
	...
	initialize: function(element, options){
		this.element = document.id(element).store('Hestrap.Popup', this);
		this.setOptions(options);
		this.bound = {
			hide: this.hide.bind(this),
			bodyClick: function(e){
				if (Hestrap.version == 2){
	...
	_checkAnimate: function(){
		...
		if (!this._canAnimate) {
			...
			if (Hestrap.version >= 3) this.element.removeClass('hide').removeClass('hidden');
			if (this._mask){
				this._mask.addClass('fade');
				if (Hestrap.version >= 3) this._mask.removeClass('hide').removeClass('hidden');
			}
		}
	},
	...
	hide: function(event, clicked){
		...
		if (Hestrap.version == 2) document.id(document.body).removeEvent('click', this.bound.hide);
	...
	_makeMask: function(){
		...
		if (this.options.closeOnClickOut && Hestrap.version == 2){

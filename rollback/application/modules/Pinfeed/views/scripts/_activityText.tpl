    ...
    <?php else : ?>
      ...
      <span
        ...
        if ($moduleTable->isModuleEnabled('hashtag')) {
            ...
            foreach($hasgtags as $tag){
              if($tag['resource_id'] == $action->getIdentity()){
                if ($this->name == $tag['hashtag']) {
                  $asd = str_ireplace('#'.$tag['hashtag'], '<a href="javascript:void(0)" onClick="click_hashtags(\'' . $url . '\',\'' . $tag['hashtag'] . '\',\''.$this->translate("WALL_RECENT").'\',\'\',\''.$action->action_id.'\', true);" title="'.$this->translate('TITLE_LINK_HASHTAG').'" style="font-weight: bold;">' . '#'.$tag['hashtag'] . '</a>', $asd);
                } else {
                  $asd = str_ireplace('#'.$tag['hashtag'], '<a href="javascript:void(0)" onClick="click_hashtags(\'' . $url . '\',\'' . $tag['hashtag'] . '\',\''.$this->translate("WALL_RECENT").'\',\'\',\''.$action->action_id.'\', true);" title="'.$this->translate('TITLE_LINK_HASHTAG').' ">' . '#'.$tag['hashtag'] . '</a>', $asd);
                }
              }
          }
            }
          ...
          echo strip_tags($action->getContent());
        }
        ?>
          ...
          <?php if ($action->attachment_count > 0 && count($action->getAttachments()) > 0): ?>
            <?php if (count($action->getAttachments()) == 1 && current($action->getAttachments())->item &&
              ...
              if($('video_thumb_'+video_id)){
               var $video<?php echo $video_id;?> = $('video_object_'+video_id).get('html');
                $('video_thumb_'+video_id).removeAttribute('onclick');

                $('video_thumb_'+video_id).addEvent('click', function(){
                  videoViewer($video<?php echo $video_id;?>);
                  $('video_object_'+video_id).setStyle('display','none');
                });
            ...
            <?php else: ?>
              ...
              foreach ($action->getAttachments() as $attachment): ?>
                ...
                <?php elseif ($attachment->meta->mode == 2 && $i<7): // Thumb only type actions ?>
                  ...
                  <div class="feed_attachment_photo">
                    ...
                    <?php
                    if($i==1 || $i==4){
                      $block_photo = 'thumb.normal';
                    }else{
                      $block_photo = 'thumb.icon';
                    }
                    echo $this->htmlLink($attachment->item->getHref(), $this->itemPhoto($attachment->item, $block_photo, $attachment->item->getTitle()), $attribs) ;?>
                  </div>

...
class Pinfeed_Installer extends Engine_Package_Installer_Module
{
  public function onPreInstall()
  {
    ...
    $hashtag = $db->fetchRow($select);

    if ($hashtag) {
      if (version_compare($hashtag['version'], '4.5.0p1') < 0) {
        $error_message = $translate->_('This plugin requires Hire-Experts Hashtag Module. We found that you has old version of Hashtag module, please download latest version of Hire-Experts Hashtag Module and install.');
    ...
    if (!$wall) {
      $error_message = $translate->_('Error! This plugin requires Hire-Experts Wall module.');
      return $this->_error($error_message);
    }

    if (version_compare($wall['version'], '4.2.6p4') < 0) {
      $error_message = $translate->_('This plugin requires Hire-Experts Wall Module. We found that you has old version of Wall module, please download latest version of Hire-Experts Wall Module and install.');
      return $this->_error($error_message);
    }

...
class Mobile_Api_Menus extends Core_Api_Abstract
{
  ...
  public function getMenuParams($name, array $options = array(), $activeItem = null)
  {
    ...
    foreach( $menu as $row )
    {
      ...
      if( !empty($row->plugin) ) {
        ...
        } else if( $result instanceof Zend_Db_Table_Row_Abstract && $result->getTable() instanceof Mobile_Model_DbTable_MenuItems ) {
  ...
  public function getMenu($name)
  {
    ...
    // Get items
    $table = Engine_Api::_()->getDbtable('menuItems', 'mobile');
    $select = $table->select()
      ->where('menu = ?', $name)

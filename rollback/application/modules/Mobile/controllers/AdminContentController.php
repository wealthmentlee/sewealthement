 ...
 * @version    $Id: AdminContentController.php 2011-02-14 06:58:57 mirlan $
 * @author     Mirlan
 */
...
class Mobile_AdminContentController extends Core_Controller_Action_Admin
{
  public function indexAction()
  {
    ...
    $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
      ->getNavigation('mobile_admin_main', array(), 'mobile_admin_main_content');
    ...
    $pageTable = Engine_Api::_()->getDbtable('pages', 'mobile');
    $contentTable = Engine_Api::_()->getDbtable('content', 'mobile');
    ...
    $select = $pageTable->select()
        ->from(array('p' => $pageTable->info('name')), new Zend_Db_Expr('p.*'))
        ->join(array('m' => $moduleTable->info('name')), 'p.module = m.name AND m.enabled = 1', array());

    // Get page list
    $this->view->pageList = $pageList = $pageTable->fetchAll($select);
  ...
  {
    if (_ENGINE_ADMIN_NEUTER) {
      return $this->_helper->redirector->gotoRoute(array('module' => 'mobile', 'controller' => 'content', 'action' => 'index'), 'admin_default', true);
    }
    // Get page param
    $page = $this->_getParam('page');
    $pageTable = Engine_Api::_()->getDbtable('pages', 'mobile');
  ...
  public function saveAction()
  {
    ...
    if (_ENGINE_ADMIN_NEUTER) {
      $form->addError('This action is disabled');
  ...
  public function getContentAreas()
  {
    ...
    $modules = Zend_Controller_Front::getInstance()->getControllerDirectory();
		$mobile = Zend_Controller_Front::getInstance()->getControllerDirectory('mobile');
    foreach( $modules as $module => $path) {
			$contentManifestFile = dirname($mobile) . '/settings/' . $module . '-content.php';
    ...
    // From widgets
    $it = new DirectoryIterator(dirname($mobile) . '/widgets/widgets');
    foreach( $it as $dir ) {

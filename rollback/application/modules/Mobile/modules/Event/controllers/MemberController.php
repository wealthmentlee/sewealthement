...
class Event_MemberController extends Core_Controller_Action_Standard
{
  ...
  public function joinAction()
  {
    ...
    if( $this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost()) )
    {
      ...
      try
      {
        $member = $subject->membership()->getRow($viewer);
        $membership_status = ($member) ? $member->active : false;  // fixed notice
  ...
  public function rsvpAction()
  {
    ...
    $rsvp = in_array($this->_getParam('rsvp'), array(0,1,2)) ? $this->_getParam('rsvp') : 0;

    $event->membership()
        ->getRow($viewer)
        ->setFromArray(array('rsvp' => $rsvp))
        ->save();

    return $this->_forward('success', 'utility', 'mobile', array(
      'messages' => array(Zend_Registry::get('Zend_Translate')->_('MOBILE_You have successfully RSVPed to this event.')),
      'return_url'=>urldecode($this->_getParam('return_url')),
    ));

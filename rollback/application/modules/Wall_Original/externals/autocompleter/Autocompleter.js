/* $Id: Autocompleter.js 18.06.12 10:52 michael $ */

if (!Wall){
  var Wall = {};
}

Wall.Autocompleter = new Class({
	...
	initialize: function(element, options) {
		...
		this.build();
		this.observer = new Wall.Observer(this.element, this.prefetch.bind(this), $merge({
	...
	build: function() {
		if ($(this.options.customChoices)) {
			...
			if(!this.options.ignoreOverlayFix) this.fix = new Wall.OverlayFix(this.choices);
		}
    ...
    if( !this.options.alwaysOpen ) {
      this.element.addEvent('focus', this.toggleFocus.create({bind: this, arguments: true, delay: 100}))

      if (this.options.elementBlur) {
        var self = this;
        this.element.addEvent('blur', function() {
          self.fireEvent('onElementBlur', [self.element]);
        });
      } else {
        this.element.addEvent('blur', this.toggleFocus.create({bind: this, arguments: false, delay: 100}));
      }
    }
	},
		...
		if (e && e.key && !e.shift && !this.options.ignoreKeys) {
          ...
          var value = this.element.value;
					if (!this.prefetch() && this.queryValue !== null) {
						var up = (e.key == 'up');
            this.break_event = true;
            var choices_box = (this.options.customContainer == null || this.choices.getElement('.' + this.options.customContainer) == null)
              ? this.choices
              : this.choices.getElement('.' + this.options.customContainer);

            this.choiceOver((this.selected || choices_box)[
    ...
    var first = (this.options.customContainer == null || this.choices.getElement('.' + this.options.customContainer) == null)
      ? this.choices.getFirst(match)
      : this.choices.getElement('.' + this.options.customContainer).getFirst(match);
...
Wall.Autocompleter.Base = Wall.Autocompleter;
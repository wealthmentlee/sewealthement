 ...
 * @version    $Id: AuthController.php 10115 2013-11-08 06:17:54Z john $
 * @author     John
 */
...
//    }
    ...
    if (!empty($_COOKIE['cookie_test'])) {
      //setcookie('cookie_test', 1, time()-10, '/');
    } else {
      $this->view->status = false;
      $this->view->error = Zend_Registry::get('Zend_Translate')->_('Browser cookies are required to login.');
      $form->addError(Zend_Registry::get('Zend_Translate')->_('Browser cookies are required to login.'));
      //setcookie('cookie_test', 1, time()-10, '/');
      return;
    }  
    ...
    if( !$form->isValid($this->getRequest()->getPost()) ) {
      ...
      if(isset($_SESSION['login_attempt'])){
        $newAttemptValue = (int) $_SESSION['login_attempt'] + 1;
        $_SESSION['login_attempt'] = $newAttemptValue;
      } else {

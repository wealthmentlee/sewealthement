 ...
 * @author adik
 */
class Like_Widget_MostLikedAlbumsController extends Engine_Content_Widget_Abstract{
    public function indexAction()
    {
        $this->view->item_type = $item_type = 'album';
        $api = Engine_Api::_()->getApi('core','like');

        $settings = Engine_Api::_()->getApi('settings','core');
        $ipp = $settings->getSetting('like.album.count',9);
        $this->view->period = $period = $settings->getSetting('like.album.period',1);

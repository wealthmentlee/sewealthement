 ...
 */
class Like_Widget_MostLikedJobsController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
    if (!Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('job')) {
      $this->setNoRender($this);
      return;
    }

    $this->view->widget = 'most_liked';
    $this->view->item_type = $item_type = 'job';
    $api = Engine_Api::_()->getApi('core', 'like');

    $settings = Engine_Api::_()->getApi('settings', 'core');
    $ipp = $settings->getSetting('like.job_count', 9);
    $this->view->period = $period = $settings->getSetting('like.job_period', 1);
